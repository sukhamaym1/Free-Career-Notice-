...
const targetFormStart = `          <form onSubmit={handleSavePost} className="space-y-4 max-w-4xl">`;
const targetFormEnd = `          </form>`;

const regex = /<form onSubmit={handleSavePost} className="space-y-4 max-w-4xl">([\s\S]*?)<\/form>/;
            ...
            <div className="flex-1 space-y-6 min-w-0">
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 space-y-4">
                ...
                <div>
                  ...
                  <textarea name="seoDescription" defaultValue={editingPost?.seoDescription || ''} rows={2} placeholder="Brief description for search results" className="w-full bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
            </div>
