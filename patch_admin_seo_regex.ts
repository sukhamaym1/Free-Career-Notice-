...
content = content.replace(/const postId = isEdit \? editingPost\.id : `post-\$\{String\(nextIdNumber\)\.padStart\(3, '0'\)\}`;\s*const newPost = \{\s*id: postId,\s*title: formData\.get\('title'\),\s*categorySlug: formData\.get\('categorySlug'\),/, replacementSavePost);
